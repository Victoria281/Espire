...
func (r *collectionSqlRepository) FindAll(username string) ([]models.Collection, error) {
	...
	err := r.DB.Preload("Articles").Preload("Articles.Links").Preload("Articles.Quotes").Preload("Articles.Flashcards").Preload("Articles.Tags").Preload("Synthesis").Where("username = ?", username).Where("deleted_at IS NULL").Find(&collections).Error
